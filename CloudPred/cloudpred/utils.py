...
def train_classifier(Xtrain, Xvalid, Xtest, classifier, regularize=None,
                     eta=1e-8, iterations=3, stochastic=True, cuda=False, state=None, regression=False,suff=''):
    ...
    #                     y_score = onehot_encoder.fit_transform(integer_encoded)
                ...
                if dataset == "valid":
                    ...
                    if res["loss"] <= best_res["loss"]:
                        ...
                        if iterations - 1 ==0:
                            with open(args.dir+'/y_score'+suff, 'wb') as fp:
                                pickle.dump(y_score, fp)
                            pred= torch.stack(pred_)
                            with open(args.dir+'/pred'+suff, 'wb') as fp:
                                pickle.dump(pred.tolist(), fp)  
                            with open(args.dir+'/y_true'+suff, 'wb') as fp:
